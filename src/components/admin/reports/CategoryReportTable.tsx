...
import { TopCategory } from "@/types/admin/report.type";
...
const CATEGORY_COLORS = [
  ...
  { bg: "bg-lime-50", text: "text-lime-700", bar: "bg-lime-400" },
  { bg: "bg-orange-50", text: "text-orange-700", bar: "bg-orange-400" },
  { bg: "bg-fuchsia-50", text: "text-fuchsia-700", bar: "bg-fuchsia-400" },
];
...
function CategoryPill({ name }: { name: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center justify-center w-8 h-8 rounded-lg text-xs font-semibold shrink-0 ${color.bg} ${color.text}`}
    >
      {initials}
    </span>
  );
...
function RevenueBar({ value, max, colorClass }: { value: number; max: number; colorClass: string }) {
...
const CategoryReportTable = ({ categories }: Props) => {
      ...
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">
            {categories.length} categories · {totalSold.toLocaleString()} units sold
          </p>
        </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-50">
            {categories.map((category, index) => {
              ...
              const soldPct = totalSold > 0 ? (category.sold / totalSold) * 100 : 0;
              const revenuePct = totalRevenue > 0 ? (category.revenue / totalRevenue) * 100 : 0;
