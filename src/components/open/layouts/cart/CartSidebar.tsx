...
export const CartSideBar = ({ isOpen, onClose, user }: Props) => {
  const { cart, loading, updateQuantity, removeItem, itemCount, loadCart } =
    useCart();
  ...
  const handleProceedToCheckout = () => {
    const target = isAuthenticated ? "/checkout" : "/checkout/guest";
  ...
  // Store cart was open before reload
  sessionStorage.setItem('cartWasOpen', 'true');
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              {[...Array(3)].map((_, i) => (
                <div
                  key={i}
                  className="flex gap-4 pb-4 border-b border-[#E5E5E5] dark:border-[#2A2A2A]"
                >
