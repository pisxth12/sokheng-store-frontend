import { publicWishlistApi } from "@/lib/api/open/wishlist";
...
export default  function WishlistProvider ({children}: {children: React.ReactNode}) {
    ...
    const checkItem = useCallback(async (productId: number): Promise<boolean> => {
        try {
            ...
            setError(err instanceof Error ? err.message : "Failed to check item in wishlist");
            console.error(err);
            return false;
        }
    }, []);
    ...
    const moveToCart = useCallback(async (productId: number, quantity: number = 1): Promise<boolean> => {
        setError(null);
        try {
    ...
    // Toggle wishlist
    const toggleItem = useCallback(async (productId: number, isInWishlist: boolean): Promise<boolean> => {
        if (isInWishlist) {
