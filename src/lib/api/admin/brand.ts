...
export const adminBrandApi = {
    ...
    addCategories: async (brandId: number, categoryIds: number[]): Promise<Brand> => {  
        const res = await apiClient.post<Brand>(`/admin/brands/${brandId}/categories`, {
            categoryIds,
        });
        return res.data;
    },

    removeCategories: async (brandId: number, categoryIds: number[]): Promise<Brand> => {  
        const res = await apiClient.delete<Brand>(`/admin/brands/${brandId}/categories`, {
            data: { categoryIds },
        });
        return res.data;
    },
