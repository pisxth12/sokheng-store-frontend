...
export const publicWishlistApi = {
    ...
    add: async (productId: number): Promise<WishlistResponse> => {
        const res = await apiClient.post<WishlistResponse>(`/wishlist/add/${productId}`);
        return res.data;
    },

    remove: async (productId: number): Promise<WishlistResponse> => {
        const res = await apiClient.delete<WishlistResponse>(`/wishlist/remove/${productId}`);
        return res.data;
    },
    ...
    moveToCart: async (productId: number, quantity: number): Promise<WishlistResponse> => {
        const res = await apiClient.post<WishlistResponse>(`/wishlist/move-to-cart/${productId}`, { quantity });
        return res.data;
    }
