import {  PaymentRequest, PaymentStatusResponse, QRGenerationResponse } from "@/types/open/payment.type";
import apiClient from "./client";
 ...
 export const publicPaymentApi = {
  ...
  verifyPayment: async (orderNumber: string, md5: string): Promise<PaymentStatusResponse> => {
    try {
  ...
  getPaymentStatus: async (orderNumber: string): Promise<PaymentStatusResponse> => {
    try {
      const response = await apiClient.get<PaymentStatusResponse>(
