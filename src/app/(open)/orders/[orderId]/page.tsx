    ...
    const startPaymentVerification = (md5: string) => {
        ...
        pollingIntervalRef.current = setInterval(async () => {
            try {
                ...
                console.log(`⏱️ Verification attempt #${verificationAttemptsRef.current}`);
                ...
                } else if (verificationAttemptsRef.current >= MAX_VERIFICATION_ATTEMPTS) {
...
const isCompleted = order?.status === 'COMPLETED' || paymentCompleted;
    ...
    const statusColor = order?.status === 'COMPLETED' ? 'text-emerald-400' : order?.status === 'PENDING' ? 'text-amber-400' : 'text-white/40';
...
/>
            ...
            <div className="border-b border-white/6 px-6 py-4 flex items-center justify-between">
                <button onClick={() => router.push('/')} className="text-white/40 hover:text-white text-sm transition-colors flex items-center gap-2">
                    <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                        <path d="M9 2L4 7L9 12" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                    </svg>
                    Home
                </button>
                <span className="mono text-xs text-white/25 tracking-widest uppercase">Order</span>
            ...
            <div className="max-w-2xl mx-auto px-6 py-10 space-y-3">
                ...
                <div className="rounded-2xl border border-white/6 bg-white/2 overflow-hidden">
                    ...
                    <div className="divide-y divide-white/4">
                        {order?.items?.map((item) => (
                            <div key={item.id} className="item-row flex  items-center gap-4 px-5 py-3.5">
                                {item.productImage && (
                                    <Link  href={`/products/${item.slug}`} className="w-10 h-10 rounded-xl overflow-hidden bg-white/5 shrink-0">
                                        <img src={item.productImage} alt={item.productName} className="w-full h-full object-cover" />
                                    </Link>
                                )}
                                <div className="flex-1 min-w-0">
                                    <p className="text-sm font-medium text-white truncate">{item.productName}</p>
                                    <p className="text-xs text-white/35 mt-0.5">${item.unitPrice} × {item.quantity}</p>
                                </div>
                                <p className="mono text-sm text-white/70 shrink-0">${item.subtotal?.toFixed(2)}</p>
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="border-t border-white/6 px-5 py-4 flex justify-between items-center">
                        ...
                        <span className="mono font-medium text-white">${order?.totalAmount?.toFixed(2)}</span>
                    </div>
                </div>
               ...
               {isPending && !paymentCompleted && (
                    ...
                    >
                        {isVerifying ? 'Processing...' : `Pay $${order?.totalAmount?.toFixed(2)} with QR`}
                    </button>
                )}
