// app/(open)/category/[slug]/CategoryClient.tsx
"use client";

import { ProductGrid } from "@/components/open/products/ProductGrid";
import { LoadingSpinner } from "@/components/ui/LoadingSpinner";
import { ScrollToTop } from "@/components/ui/ScrollToTop";
import { useCategoryProducts } from "@/hooks/open/useCategoryProducts";
...
export default function CategoryClient({  category, slug, initialData }: CategoryClientProps) {
  const { products, loading, hasMore, total, error, loadMore, reset } =
    useCategoryProducts(slug, initialData);
        ...
        <>
          ...
          {!hasMore && products.length > 0 && (
            <p className="text-center text-gray-500 py-8">
              🎉 You've viewed all {total} products in this category
            </p>
          )}
        </>
      ) : (
